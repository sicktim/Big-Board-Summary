...
| D11 | **v0.5**: MCG now ships as a separate `mcg-26a.json` file next to `index.html`, loaded via fetch at boot. Replaces D8's embedded-fence approach. Required for the new ~1.4MB handoff JSON (kept the HTML editable) and works cleanly on GitHub Pages. Needs a local HTTP server for `file://` dev (Python/Node both one-liner) | v0.5 rewrite |
| D12 | **v0.5**: Board-student fields `type` (Pilot-F/M/B → `P`) and `dataGroup` (`LJ-25` → `Learjet`) are normalized inside the DAG builder to match MCG applicability codes. Central helpers `viewerTypeOf(student)` / `viewerDGOf(student)` | v0.5 implementation |
| D13 | **v0.5**: Class view is a **union of per-student DAGs** across all goal-scope students, not a single MCG walk. Ensures every student's specific prereq chain (crew-solo/non-crew-solo, DG-specific) surfaces in the graph if it applies to anyone | KB §9; v0.5 implementation |
| D14 | **v0.5**: Rule D/E (crew-solo / non-crew-solo) are **pilot-only**. For non-pilot viewers, the `nameTagRole` is a no-op — they take the event if they're in the nameTag. Fixed a pruning bug where FTEs/ABMs/CSOs got over-pruned at events with non-crew-solo aircraft tags covering all standard types (e.g., CF 6521E `[ABM,CSO,FTE,P,RPA]` non-crew-solo C-12) | MCG agent audit; fixed in v0.5 |
| D15 | **v0.5**: `forDownstream` with `target === hostEvent` is treated as a **tautology** (kept unconditionally). Non-code sentinel targets (`"all"`, `"P"`, or descriptive phrases) are **kept unconditionally** (conservative; awaits explicit semantics) | KB §7.1 open item |
| D16 | **v0.5**: Rule G tight-serial collapse (`seriesInfo.inSeries` + position > 1) applied at graph-build time. Position N keeps ONLY a single edge to position N-1, replacing the duplicated foundational list that the MCG preserves | KB §10; Rule G §6 |
| D17 | **v0.5**: `oneOf` prereq groups collapse into a **virtual "pick one of" node** only when *more than one* alternative survives the per-viewer filter. If only one survives, a direct edge is drawn — no group UI clutter | KB §5 oneOf table; v0.5 implementation |

### New open items (v0.5)

- **R8** — `oneOf` audience-specific routing. For FQ 6230F (glider), both CF 6203F and CF 6215F survive the filter for any pilot; the KB hint is "non-solo students take CF 6203F, select-P solo pilots take CF 6215F" but we don't have a `select-P` marker on individual students. Current behavior: show both via virtual group node. User confirmation needed on whether that's the right UI or whether we need per-student select-P tagging.
- **R9** — Dangling prereq `TF 5301A` (cited by TF 5320A, no matching event in 26A). Currently traversed-and-collapsed as an off-board dead-end. If anyone's chain routes through TF 5320A, the dangling edge is dropped silently.
- **R10** — BB-only codes not in MCG (6 observed: CF 6306H, CF 6380M, CF 6381F, CF 6690M, CF 6691F, FQ 8203F). These cells still render on the board but don't connect to any chain. Flag in future issues.md pass if they block a user workflow.
- **R11** — DG for Pilot-M / Pilot-B is unresolved in the KB. My current code treats `dataGroup` on the board as ground truth. This may need revisiting when Pilot-M (multi-engine) / Pilot-B (bomber) board rosters map to specific MCG DGs.
