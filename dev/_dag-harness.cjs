...
function aggregateEvent(evt, mcgEvent, perStudent, students) {
  // Board data is sole source of truth: dark-grey = not required.
  const apps = [];
  for (const s of students) {
    const ps = perStudent[s.col];
    ps.applicable = ps.completion !== 'notreq';
...
function isOpted(goalCode, payload, student, mcgEvent) {
  // Board data check: dark-grey cell on goal = not required.
...
function layoutDag(dag, payload, { mode, student }) {
  ...
  for (const code of Object.keys(dag.nodesByCode)) {
    ...
    } else if (dag.nodesByCode[code].depth !== 0) {
      const { agg } = computeNodeAggregate(code, payload);
      if (agg.applicable === 0) continue;
    }
    visible.add(code);
...
function goalScopeStudents(payload, goalCode) {
  const goalEvt = payload.byCode[goalCode];
  if (!goalEvt) return payload.students.slice();
...
// (2) layoutDag with scopeStudents — v0.4.3 signature. Board-data-driven
// applicability: dark-grey = not required, anything else = applicable.
...
// Spot checks — board-data-driven scope (v0.4.3).
// PF 8332F: all ABM/FTE/P have non-dark-grey cells → 21.
const sPF8332F = scopeByGoal['PF 8332F'];
check('PF 8332F scope count === 21', sPF8332F.length === 21, { got: sPF8332F.length });
// PF 8311F: MCG says CSO/RPA (3), but board has 3 FTEs with paired cells
// (E KNOERR, E OREN, E DOBBS) → board wins → 6.
const sPF8311F = scopeByGoal['PF 8311F'];
check('PF 8311F scope count === 6', sPF8311F.length === 6, { got: sPF8311F.length });
