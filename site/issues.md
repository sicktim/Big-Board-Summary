...
- When hovering over a prerequisites not met person "WARN: prereqs not met" with the scheduled date. List what prerequisites are missing.
    ...
    - **RESOLVED v0.5.2c (2026-05-04)**: class-view roster dots whose
      state is "prereqs not met" (scheduled-not-opted, paired-not-opted,
      or plain pending-not-opted) now show a multi-line native tooltip
      in the user's 2026-05-04 format:
          WARN: PreReqs Not Met
          Sched - 5/14
          Missing:
          * PF 8290 - Mission Title
          * PF 3670 - Mission Title
      Date line omitted when not scheduled; substitutes "Paired w/
      <partner>" for paired-opted. Missing list capped at 12 with
      "+ N more". Implemented in renderRoleCol via chain.missingPrereqs.

v0.5.2 (continued)
- **2026-05-05** — Duplicate-event handling settings don't apply.
    - "Any of selected" mode does nothing — user marked T-38 TP
      not-required for one student and F-16 TP complete for that same
      student (same event code on two BB rows), expected the merged
      event to show "complete" but it still flagged a MCG/DBB mismatch.
    - "All of selected" — no sub-events shown.
    - "One of selected" — uses the first selected row instead of
      letting the user pick which row.
    - Banner: needs an × close button or auto-dismiss when Settings
      opens.
    - **RESOLVED v0.5.2f (2026-05-05)**: applyDupResolution was a
      no-op stub from v0.5.2a — replaced with real implementation.
      'one' uses the single selected row (UI now renders radios so
      that's literal); 'any' synthesizes a merged event whose per-
      student cell is the best status across selected rows (complete
      > scheduled > paired-opted > past-dated > blank > excused).
      'all' falls back to 'any' with a console warning until v0.5.3
      adds proper sub-event splitting (requires synthesizing codes
      and rewiring MCG edges — non-trivial). applyDupResolution now
      runs on initial load so persisted settings stick. Banner gets
      an × close + auto-hide on CTA click.

- **2026-05-05** — Past events not verified should render identically to
  future-scheduled events. Today (5/5), Ryan's PF 8240F (sched 5/4,
  unmarked on the board) flipped from yesterday's yellow "scheduled
  5/4" dot to today's pending (red-bordered white) dot. Nothing changed
  on the board — only the calendar moved. The visual should still be
  yellow; only the wording should hint at "verify."
    - **RESOLVED v0.5.2e (2026-05-05)**: classifyCell now emits
      completion='scheduled' for any white cell with a date, past or
      future. Past-dated cells get flag='past-date-unchecked' which
      drives "verify" wording in tooltips/chips but doesn't change the
      dot color or chip band. Chain walker simplified to `status !==
      'pending'` for descent termination. Class-view tooltip appends
      "— verify"; student-view chip appends "· VERIFY". Per-prereq
      detail strings updated to keep the same logic.

- **2026-05-04** — Hard WARN over-fires when a prereq was scheduled in
  the past but not yet marked complete on the board. Specifically:
  Morrison scheduled PF 8241F on 5/4, PF 8240F was scheduled 5/1
  (probably done, IP just hasn't updated the board). Both events
  render as hard red WARN; PF 8240F shows "not started." This is
  normal grading lag, not a planning hole — should be a soft monitor
  ("scheduled back on 5/1, check completion status"), not a hard
  warning.
    - **RESOLVED v0.5.2d (2026-05-04)**: split prereq-not-met into
      three categories — hard (any missing prereq has no schedule),
      soft-past (all scheduled, at least one past-dated awaiting
      update), soft-future (all scheduled in the future). New
      `.dot.warn-soft` (amber, no pulse) and `.node.s-soft` band
      (sand/amber chip) for the soft cases. Tooltip headers switch
      per category. Chain walker also stops descending into
      past-dated white cells so a blank prereq three levels upstream
      of a presumed-done flight doesn't escalate the chain.

v0.5.2
- PF 8211F shows "no applicable students". However, MCG PDF shows ABM/CSO/FTE, json shows the same (verify i'm looking at right spot), and the event tab reflects that. Digital big board shows this event just like MCG. I can't figure out why this one is showing "no applicable students".
    - Did more digging. Looks like on the Big Board the one CSO mistankingly had a dark grey container denoting he didn't need it, however that was falsely color coded and he needs it. This demonstrates that the code looks for the color to determine applicability. In this case I would have expected the code to cross reference the MCG applicability (authoritative), against my student roster to find CSO/ABM/FTEs and list their requirement. Furthermore, in this case I would expect an error notification to notify me of applicability mismatch between MCG and DBB.
    - **RESOLVED v0.5.2b (2026-05-03)**: aggregateEvent now tracks
      `mcgApplicable` and `excused` separately. Roster always lists
      students MCG says are required; dark-grey-on-board students stay
      visible as "excused" (hatched dot, italic grey name, ⚠ prefix) and
      a yellow node-level mismatch banner names them. KB §0 added
      ("Source-of-truth ordering") and AGENTIC.md §System Evolution
      records this as the second occurrence of "code drifted into using
      DBB color as authority over MCG."
- PF 8241F syllabus deviation warning incorrectly shown.
    - Both Pope and Sternat have completed their PF 8240F (and it shows green on the panel), however it shows they haven't met prerequisites.
    - **RESOLVED v0.5.2b (2026-05-03)**: `chain.buildChainForStudent`
      walked all prereqs without per-student filtering or Rule G series
      collapse, so it descended into PF 8240F's foundation prereqs even
      though the student had completed the higher-level event. Lifted
      `edgePassesForViewer` + `seriesPrevPrereq` to a top-level
      `dagFilter` module; chain now (a) applies the same Rule A /
      nameTag / DG / datagroup edge filter as the renderer's DAG
      builder, (b) honors series collapse, and (c) early-outs when the
      student has already completed a chain event (its prereqs are
      implicit).
- Paired events - Status that just shows paired events isn't very helpful. It would be helpful if it showed status just like other events and color-coded pair as colored font for their names (or easier would be (P1), (P2) following their names).
    - **RESOLVED v0.5.2b (2026-05-03)**: paired-opted is now a real
      status. Class view: dot stays purple, name gets a "(paired w/
      <partner>)" suffix in purple — partner identified by matching
      `bgHex` within the same event row (each pair shares a unique cell
      color). Student view: chip bands `PAIRED · SCHEDULED` (date),
      `PAIRED · OPTED` (no date, prereqs met), `⚠ PAIRED · NOT OPTED`
      (prereqs missing). Tooltip distinguishes the four sub-states.